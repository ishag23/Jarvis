...
JARVIS_WELCOME_PHRASE = (
    ...
    "Today is Saturday, 15th August. Happy Independence Day, by the way. "
    "The weather is a bit warm today, I feel. "
    "Let me open your project you were working on?"
)
...
# --- voice command (spoken after the welcome) -------------------------------
# After the welcome finishes, Jarvis listens for a spoken command. If it hears a
# project-open request ("open my project", "localhost", "4200"), it opens PROJECT_URL.
VOICE_COMMAND_ENABLED = True
PROJECT_URL = "http://localhost:4200"
# Keywords that count as "open my project" (matched case-insensitively).
PROJECT_COMMAND_KEYWORDS = ("project", "localhost", "4200", "open my", "dev server")
COMMAND_START_TIMEOUT_S = 5.0   # give up if no speech starts within this many seconds
COMMAND_MAX_SECONDS = 6.0       # hard cap on how long a single command can be
COMMAND_SILENCE_S = 1.2         # stop recording after this much trailing silence
COMMAND_START_RMS = 0.02        # speech must exceed this RMS to start capturing
COMMAND_SILENCE_RMS = 0.012     # below this counts as silence while capturing
...
def elevenlabs_speak(text: str) -> None:
    """Speak an arbitrary line via ElevenLabs (no caching). Blocks until playback ends."""
    text = (text or "").strip()
    if not text:
        return
    vid, model_id, output_format, pcm_rate = elevenlabs_env_config()
    ...
    if not vid or not api_key:
        log.warning("ElevenLabs voice/key not set; cannot speak %r", text)
        return
    try:
        from elevenlabs.client import ElevenLabs
...
def _record_command_audio(device: int, blocksize: int) -> np.ndarray | None:
    """Record from the mic until trailing silence (or a timeout). Returns float32 mono, or None."""
    ...
    try:
        with sd.InputStream(
            ...
            while True:
                ...
                now = time.monotonic()
                if not started:
                    if level >= COMMAND_START_RMS:
                        started = True
                        frames.append(np.array(data, copy=True))
                    elif now - start > COMMAND_START_TIMEOUT_S:
                        return None
                else:
                    frames.append(np.array(data, copy=True))
                    if level < COMMAND_SILENCE_RMS:
                        silence_run += block_s
                        if silence_run >= COMMAND_SILENCE_S:
                            break
                    else:
                        silence_run = 0.0
                    if now - start > COMMAND_MAX_SECONDS:
                        break
    except sd.PortAudioError as e:
        log.warning("Command capture failed: %s", e)
        return None
...
def _transcribe_command(audio_f32: np.ndarray) -> str:
    """Transcribe recorded audio via ElevenLabs speech-to-text."""
    flat = np.clip(audio_f32.reshape(-1), -1.0, 1.0)
    pcm_i16 = (flat * 32767.0).astype(np.int16)
    ...
    with open(path, "rb") as f:
        result = client.speech_to_text.convert(file=f, model_id=STT_MODEL_ID)
    return (getattr(result, "text", "") or "").strip()


def open_project() -> None:
    """Open the local project URL (e.g. a dev server on localhost:4200)."""
    _open_url_in_chrome(
        PROJECT_URL,
        new_window=True,
        label="Project",
...
def listen_and_handle_command(device: int, blocksize: int) -> None:
    """After the welcome, capture one spoken command and act on it."""
    if not VOICE_COMMAND_ENABLED:
        return
    time.sleep(0.3)  # let the welcome playback tail settle before recording
    log.info("Listening for a command...")
    audio = _record_command_audio(device, blocksize)
    if audio is None:
        log.info("No command heard.")
        return
    try:
        text = _transcribe_command(audio)
    except Exception as e:
        log.warning("Speech-to-text failed: %s", e)
        return
    if not text:
        log.info("Command was empty after transcription.")
        return
    log.info("Heard command: %r", text)
    low = text.lower()
    if any(kw in low for kw in PROJECT_COMMAND_KEYWORDS):
        log.info("Opening project: %s", PROJECT_URL)
        elevenlabs_speak("Opening your project now, ma'am.")
        open_project()
    else:
        log.info("Command not recognized as a project-open request.")
        elevenlabs_speak(" ")


def run_double_clap_actions(device: int, blocksize: int) -> None:
    """Run outside the mic loop so sleeps do not stall capture."""
    try:
        # Speak the welcome first...
        if JARVIS_WELCOME_ENABLED and JARVIS_WELCOME_PHRASE.strip():
            say_jarvis_welcome()
        # ...then open the tabs / focus apps.
        play_song(SONG_URI)
        open_claude_in_chrome()
        open_binance_btc_in_chrome()
        open_cursor_window()
        # Finally, listen for a spoken command (e.g. "open my project").
        listen_and_handle_command(device, blocksize)
    except Exception:
        log.exception("Double-clap action sequence failed")
        ...
        with sd.InputStream(
            ...
            while True:
                ...
                if (
                    ...
                    spike_armed = False
                    log.info(
                        "Clap detected (rms=%.5f, threshold=%.5f) — %s",
                        level,
                        threshold,
                        "waiting for second clap" if first_clap_time is None else "second clap",
                    )
